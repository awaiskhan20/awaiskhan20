<p>
  <img src="banner.webp" alt="Banner" width="100%" />
</p>

<h1>Hi 👋, I'm Awais Khan</h1>

<h3>
  WordPress Developer • WooCommerce Expert • Frontend Specialist <br/>
  🇵🇰 Based in Pakistan
</h3>

<p>
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&center=true&width=500&lines=WordPress+Specialist;Frontend+Developer;WooCommerce+Expert;Building+Fast+%26+Scalable+Websites" />
</p>

<p>
  <a href="https://www.linkedin.com/in/awais-khan-487203308/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="mailto:awaiskhan.raees123@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-Mail%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  &nbsp;
  <img src="https://komarev.com/ghpvc/?username=awaiskhan20&style=for-the-badge&color=blue&label=Profile+Views"/>
</p>

---

## 👨‍💻 About Me

> I don&#39;t just build websites &mdash; I build experiences that perform.

- 🔧 5+ years of experience in WordPress & WooCommerce  
- 🧠 Expert in custom themes, plugins & advanced customization  
- 🔌 Strong experience with REST APIs & third-party integrations  
- ⚡ Performance optimization & clean, scalable code  
- 🎯 Focused on UI/UX that converts visitors into customers  
- 🌱 Currently learning React & Headless WordPress  

---

## 🛠️ Tech Stack

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,php,wordpress,bootstrap,tailwind,git,github,vscode&theme=light" />
</p>

---

## 🧰 Tools & Workflow

- 🧩 WordPress, WooCommerce, ACF, Elementor  
- 🎨 Figma, Adobe XD  
- 💻 XAMPP, LocalWP, cPanel  
- 🔄 Git, GitHub, FTP  
- 🔍 SEO & Performance Optimization  

---

## 📊 GitHub Stats

<p>
  <img src="https://github-readme-stats.vercel.app/api?username=awaiskhan20&show_icons=true&theme=tokyonight&hide_border=true" width="48%" />
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=awaiskhan20&layout=compact&theme=tokyonight&hide_border=true" width="48%" />
</p>

<p>
  <img src="https://streak-stats.demolab.com?user=awaiskhan20&theme=tokyonight&hide_border=true" width="60%" />
</p>

---

## 🚀 Featured Projects

| Project | Description |
|--------|------------|
| 🛒 WooCommerce Quote Builder | B2B request system using AJAX + ACF |
| 🔐 Membership Paywall | Subscription-based content system |
| 🔄 CRM API Sync | WordPress ↔ CRM real-time integration |
| 🎨 Elementor Portfolio Theme | Fast & SEO-optimized portfolio |

---

## 📫 Connect With Me

<p>
  <a href="https://www.linkedin.com/in/awais-khan-487203308/">
    <img src="https://img.shields.io/badge/LinkedIn-Awais%20Khan-blue?style=for-the-badge&logo=linkedin"/>
  </a>
  &nbsp;
  <a href="mailto:awaiskhan.raees123@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail"/>
  </a>
</p>

---

<p>
  ⭐ <b>"Great code is silent, but powerful."</b> ⭐
</p>
